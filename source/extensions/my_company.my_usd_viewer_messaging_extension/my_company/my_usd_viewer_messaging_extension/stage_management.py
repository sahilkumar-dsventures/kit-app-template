...
class StageManager:
    ...
    def __init__(self):
        ...
        incoming = {
            ...
            'simulationControlRequest': self._on_simulation_control,
        }
    ...
    def _on_simulation_control(self, event: carb.events.IEvent):
        """
        Handler for `simulationControlRequest` event.
        
        Controls the simulation timeline based on the requested action.
        """
        action = event.payload.get("action")
        carb.log_info(f"Received simulation control request: {action}")
        ...
        if action == "start":
            ...
            carb.log_warn(f"Unknown simulation action requested: {action}")
